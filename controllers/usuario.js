...
const httpUsuario = {
  ...
  recuperarPassword: async(req,res)=>{
    ...
    const mailOptions = {
      ...
      text: 'Haz clic en el siguiente enlace para restablecer tu contraseña: http://localhost:5173/#/recuperar-password?token=' + token,
    };
  
    transporter.sendMail(mailOptions, (error, info) => {
      if (error) {
        console.error(error);
        res.status(500).json({ success: false, msg: 'Error al enviar el correo electrónico.' });
      } else {
  ...
  putCambioPassword: async (req, res) => {
    ...
    try {
      ...
      const validPassword = bcryptjs.compareSync(String(password), String(passAnterior));
      ...
      await Usuario.findByIdAndUpdate(usuario.id, { password:cryptNewPassword}, {new: true})
      ...
      console.log(error);
      return res.status(500).json({ msgError: "Error interno del servidor", error });
    }
  },
